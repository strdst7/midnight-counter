/**
 * Deploy midnight-counter contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
// Derive a stable 32-byte secret key from the wallet seed. It backs the
// localSecretKey() witness and never leaves this machine.
const secretKey = new Uint8Array(Buffer.from(SEED, 'hex').subarray(0, 32));

// The counter declares a `localSecretKey` witness, so the Contract constructor
// requires it to be present (even though our contract has no constructor that
// calls it). Provide a real witness instead of vacant witnesses.
const compiledContract = CompiledContract.make('counter', Counter.Contract).pipe(
  // @ts-expect-error — witnesses typed against the dynamically-imported contract (any).
  CompiledContract.withWitnesses({
    localSecretKey: ({ privateState }: any) => [privateState, privateState?.secretKey ?? secretKey],
  }),
